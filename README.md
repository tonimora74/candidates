...
The application automatically loads the correct file based on the `NODE_ENV` variable. This allows you to manage different settings (such as ports, CORS origins, API keys, etc.) for each environment without changing the code.
...
```
This is managed globally using an Express middleware in `main.ts`.
...
- Coverage for business logic files is high (>90%).
...
- Work with Monorepo for this case
...
## Test
- Unit tests implemented for main components and dialogs.

## How to Run the Frontend

```bash
cd ./prueba-angular-21
npm i
npm start
```
